...
import { useAuthToken, useUser, initializeUserSession } from '~/composables/useAuth';
...
const isOwner = computed(() => {
    return user.value && room.value && user.value.role === 'owner' && user.value.id === room.value.owner_id;
});
...
const goToEdit = () => Swal.fire('編輯', '這裡將來可以跳轉到編輯頁面', 'info');
